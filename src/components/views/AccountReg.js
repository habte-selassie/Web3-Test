...
function AccountReg(props) {
  ...
  const register = async () => {
    try {
      ...
      if (response.data.response) {
        message.success(t("Account successfully created"));
      ...
      message.error(t("Failed to register account"));
    }
  };
...
//     if (Object.values(fieldsError).some(field => field)) {
//       // If there are validation errors, return true
//       return true;
//     } else {
//       // If there are no validation errors, trigger email verification
//       verifyEmail();
//       // Return false to indicate that there are no errors
//       return false;
//     }
//   }
...
//               suffix={<a onClick={() => hasErrors(form.getFieldsError())} className="bg-gray-800 text-md text-white rounded-xl py-1 px-4">
//               {t("Send")}</a>}

//               // <a onClick={()=>hasErrors(form.getFieldsError())}  className="bg-gray-800 text-md  text-white  rounded-xl py-1 px-4">{t("Send")}</a>} 
//               className="rounded-lg  bg-gray-200"
//               onChange={(e)=>setVerificationCode(e.target.value)}/>
//             </Form.Item>
